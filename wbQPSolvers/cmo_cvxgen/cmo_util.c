...
float cmo_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*cmo_ran1(idum, reset)-1.0;
      v2 = 2.0*cmo_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double cmo_randn(void) {
  return cmo_randn_internal(&cmo_global_seed, 0);
}
void cmo_reset_rand(void) {
  srand(15);
  cmo_global_seed = 1;
  cmo_randn_internal(&cmo_global_seed, 1);
}
