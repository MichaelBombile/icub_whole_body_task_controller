...
float cop_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*cop_ran1(idum, reset)-1.0;
      v2 = 2.0*cop_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double cop_randn(void) {
  return cop_randn_internal(&cop_global_seed, 0);
}
void cop_reset_rand(void) {
  srand(15);
  cop_global_seed = 1;
  cop_randn_internal(&cop_global_seed, 1);
}
