...
float com_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*com_ran1(idum, reset)-1.0;
      v2 = 2.0*com_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double com_randn(void) {
  return com_randn_internal(&com_global_seed, 0);
}
void com_reset_rand(void) {
  srand(15);
  com_global_seed = 1;
  com_randn_internal(&com_global_seed, 1);
}
