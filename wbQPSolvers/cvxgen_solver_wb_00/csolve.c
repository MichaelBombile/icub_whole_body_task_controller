/* Produced by CVXGEN, 2019-04-22 12:50:57 -0400.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 65) && (mxGetN(xm) == 1))) {
      printf("Qw must be size (65,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 65) && (mxGetN(xm) == 1))) {
      printf("b3 must be size (65,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  *mxGetPr(xm) = work.converged;
  ...
  xm = mxCreateDoubleMatrix(65, 1, mxREAL);
