...
WlkData_logging::WlkData_logging()
{
...
void WlkData_logging::InitializeLogger()
{
    ...
    flog_Pattern.open(logPatterns.c_str());
    flog_FeetTraj.open(logFeetTrajectories.c_str());
    flog_JtsPos.open(logJointsPosValues.c_str());
    flog_F_FT.open(logFeetForceTorques.c_str());

    SimTime = 0.0;
}


void WlkData_logging::Write_Data(double SamplingTime,
                              int Cycle_counter,
                              ...
                              VectorXd CoM_position)
{
    ...
    // 
    flog_FeetTraj <<  FtTraj.StanceIndicator[0] <<"  "<< 0.0 <<"   "<<  0.0 <<"   "<< std::endl;
...
void WlkData_logging::Close_files()
{
    std::cout << " WlkData_logging Start Closing " << std::endl;
    // close the files
    flog_Pattern.close();
    flog_FeetTraj.close();
    flog_JtsPos.close();
