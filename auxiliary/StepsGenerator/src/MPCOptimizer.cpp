...
void CpQMatrix::Init(MpcBased_CP_Model MpcModel, double GainsArray[], int CpOption)
{
   ...
   double rho   = GainsVec[8]; // Footsteps planning translation
   double rho_R = GainsVec[9]; // Footsteps planning rotation
   ...
   q11trl_x = alp * (MpcModel.TheP).transpose()* MpcModel.TheP 
            + gam * (MpcModel.PEu).transpose() * MpcModel.PEu  //+ rho * (MpcModel.PEu).transpose() * MpcModel.PEu
   ...
   if (CpModelOption == 2)
    ...
    q11Ang = alp_R * Eye_Nu
            + gam_R * (MpcModel.PzuR).transpose() * MpcModel.PzuR //+ rho_R *(MpcModel.PzuR).transpose() * MpcModel.PzuR
...
MatrixXd CpQMatrix::getQMx(MpcBased_CP_Model MpcModel, CP_SelectionMatrices SMx)
{
...
MatrixXd CpQMatrix::getQMxy(MpcBased_CP_Model MpcModel, CP_SelectionMatrices SMx)
{
    MatrixXd Q12x, Q13x, Q21x, Q22x, Q23x, Q31x, Q32x, Q33x;

    MatrixXd Eye_Nu(SMx.GlobalFuture_Fep_t_X.cols(),SMx.GlobalFuture_Fep_t_X.cols());
    Eye_Nu.setIdentity();

    Q12x = -GainsVec[2] * (MpcModel.PEu).transpose() * SMx.GlobalFuture_Fep_t_X;  // g
    Q13x = -(GainsVec[2] + 0.0*GainsVec[8]) * (MpcModel.PEu).transpose() * SMx.GlobalFuture_FeE_t_X;
    // Q13x = -(GainsVec[2]) * (MpcModel.PEu).transpose() * SMx.GlobalFuture_FeE_t_X;
    Q21x =  Q12x.transpose();
    Q22x =  GainsVec[2] * (SMx.GlobalFuture_Fep_t_X).transpose() * SMx.GlobalFuture_Fep_t_X + GainsVec[8] * Eye_Nu;
    Q23x =  GainsVec[2] * (SMx.GlobalFuture_Fep_t_X).transpose() * SMx.GlobalFuture_FeE_t_X;
    Q31x =  Q13x.transpose();
    Q32x =  Q23x.transpose();
    Q33x =  (GainsVec[2] + 0.0*GainsVec[8]) * (SMx.GlobalFuture_FeE_t_X).transpose() * SMx.GlobalFuture_FeE_t_X;
    ...
    Q13y = -(GainsVec[2] + 0.0*GainsVec[8]) * (MpcModel.PEu).transpose() * SMx.GlobalFuture_FeE_t_Y;
    // Q13y = -(GainsVec[2]) * (MpcModel.PEu).transpose() * SMx.GlobalFuture_FeE_t_Y;
    Q21y =  Q12y.transpose();
    Q22y =  (GainsVec[2]) * (SMx.GlobalFuture_Fep_t_Y).transpose() * SMx.GlobalFuture_Fep_t_Y + GainsVec[8] * Eye_Nu;
    Q23y =  GainsVec[2] * (SMx.GlobalFuture_Fep_t_Y).transpose() * SMx.GlobalFuture_FeE_t_Y;
    Q31y =  Q13y.transpose();
    Q32y =  Q23y.transpose();
    Q33y =  (GainsVec[2] + 0.0*GainsVec[8]) * (SMx.GlobalFuture_FeE_t_Y).transpose() * SMx.GlobalFuture_FeE_t_Y;
 ...
 MatrixXd CpQMatrix::getQMang(MpcBased_CP_Model MpcModel, CP_SelectionMatrices SMx)
 {
...
//                        ( GainsVec[2] * (SMx.LocalFutureSel).transpose() * SMx.LocalFutureSel).cols()) = GainsVec[2] * (SMx.LocalFutureSel).transpose() * SMx.LocalFutureSel;
      ...
      Qang.block(n2, n2,nt,nt) =  GainsVec[6] * (SMx.LocalFutureSel).transpose() * SMx.LocalFutureSel + GainsVec[9] * Eigen::MatrixXd::Identity(3,3);

      return Qang;

 }

 void CpQMatrix::QmxElements(MpcBased_CP_Model MpcModel, CP_SelectionMatrices SMx)
 {
...
CpPVectorQP::CpPVectorQP(){};
CpPVectorQP::~CpPVectorQP(){};

void CpPVectorQP::Init(MpcBased_CP_Model MpcModel, double GainsArray[], int CpOption)
{
    ...
    double gam_R = GainsVec[6]; // Ztrk : Output tracking
    double kap_R = GainsVec[7]; // AVel : Average velocity tracking
    double rho_R = GainsVec[9]; // Ztrk : planning Output tracking
    // Type of LIPM model
    CpModelOption = CpOption;
    ...
    P1Ang = bet_R * (MpcModel.PvuR).transpose() * MpcModel.PvsR
            + gam_R * (MpcModel.PzuR).transpose() * MpcModel.PzsR //+ rho_R * (MpcModel.PzuR).transpose() * MpcModel.PzsR 
...
//       }
       ...
       //    VectorXd Pk1, Pk2, Pk3, fpx, PK;
           ...
           if (CpModelOption == 0||CpModelOption == 1)
             {
               // x
               VectorXd Fpx    = SMx.Fep_t * SMx.CP_GlobalCurrentSelX* CoPRef.CoPRefX - MpcModel.PEs * (St.StatesX + St.StatesDistX);
               VectorXd F1m_px = SMx.Fep_t * SMx.CP_GlobalCurrentSelX* CoPRef.CoPRefX + SMx.Fep_t * SMx.CP_GlobalFutureSelX * VeloRef.FutureStepsXY - MpcModel.PEs * (St.StatesX + St.StatesDistX);

               Pk1x =  -GainsVec[2]* (MpcModel.PEu).transpose() * Fpx -0.0*GainsVec[8]* (MpcModel.PEu).transpose() * F1m_px
               ...
               Pk2x = GainsVec[2]* (SMx.Fep_t * SMx.CP_GlobalFutureSelX + SMx.FeE_t * SMx.RotX3).transpose() * Fpx - 1.0*GainsVec[8] * VeloRef.FutureStepsXY;
               Pk3x = GainsVec[2]* (SMx.FeE_t* SMx.RotX33).transpose()* Fpx + 0.0*GainsVec[8]* (SMx.FeE_t* SMx.RotX33).transpose()* F1m_px;
               
               // y
               VectorXd Fpy   = SMx.Fep_t * SMx.CP_GlobalCurrentSelY* CoPRef.CoPRefY - MpcModel.PEs * (St.StatesY +  St.StatesDistY);
               VectorXd F1m_py= SMx.Fep_t * SMx.CP_GlobalCurrentSelY* CoPRef.CoPRefY + SMx.Fep_t * SMx.CP_GlobalFutureSelY * VeloRef.FutureStepsXY - MpcModel.PEs * (St.StatesY + St.StatesDistY);

               Pk1y =  -GainsVec[2]* (MpcModel.PEu).transpose() * Fpy -0.0*GainsVec[8]* (MpcModel.PEu).transpose() * F1m_py
               ...
               Pk2y = GainsVec[2]* (SMx.Fep_t * SMx.CP_GlobalFutureSelY + SMx.FeE_t * SMx.RotY3).transpose()*Fpy - 1.0*GainsVec[8] * VeloRef.FutureStepsXY;
               Pk3y = GainsVec[2]* (SMx.FeE_t* SMx.RotY33).transpose()* Fpy + 0.0*GainsVec[8]* (SMx.FeE_t* SMx.RotY33).transpose()* F1m_py;
...

   {
       ...
       VectorXd  LocalCurrentSel_CoPRefR = (SMx.LocalCurrentSel * CoPRef.CoPRefR);
...
//       }
       ...
       // Angular component related to the tracking of the ref. CoP orientation
       PkAng1 = P1Ang * (St.StatesR + St.StatesDistR) 
               - GainsVec[5] * (MpcModel.PvuR).transpose() * VeloRef.WzAbs
               - (GainsVec[6] + 0.0*GainsVec[9]) * (MpcModel.PzuR).transpose() * LocalCurrentSel_CoPRefR - 0.0*GainsVec[9] * (MpcModel.PzuR).transpose() * SMx.LocalFutureSel * VeloRef.FutureAngles;
       ...
       PkAng2 = -GainsVec[6] * (SMx.LocalFutureSel).transpose() * (MpcModel.PzsR * (St.StatesR + St.StatesDistR)
                - (SMx.LocalCurrentSel * CoPRef.CoPRefR)) - GainsVec[9] * VeloRef.FutureAngles;
...
CpConstraintsFootsteps::CpConstraintsFootsteps(){}

CpConstraintsFootsteps::~CpConstraintsFootsteps(){}


void CpConstraintsFootsteps::Init(CpStanceFootPose CoPref,
                                              int SptFt[],
                                                int CtrlH,
                                         VectorXd S_Cnstr)
 {
                                                                  ...
                                                                  int SptFt[])
 {
      ...
      MSetZmp = MatrixXd::Zero(18+6, 2*nu+nu+6+2+3);
...
// ===================================================================================
CpConstraintsOutputZmp::CpConstraintsOutputZmp(){}
CpConstraintsOutputZmp::~CpConstraintsOutputZmp(){}

void CpConstraintsOutputZmp::Init(  MpcBased_CP_Model MpcModel,
