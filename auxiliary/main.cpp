...
int main(int argc, char *argv[])
{
	...
	for(int i=0; i<5; i++) 
	{
		...
		for(int j=0; j<4; j++) 
		{
			ref_p_wb.p[j] 	=   wld_T_b.block<3,3>(0,0) * des_ee_pos_b[j] + wld_T_b.block<3,1>(0,3);  	// position	
			ref_o_wb.o[j] 	=   dc2quat(wld_T_b.block<3,3>(0,0) * des_ee_rot_b[j]);						// orientation
		}
		...
		cout <<" REF LEFT  FOOT IN WORLD is :	\n " << ref_p_wb.p[2] << endl;
		cout <<" REF RIGHT FOOT IN WORLD  is :	\n " << ref_p_wb.p[3] << endl;

		// update the base frame with respect to the world.
		wld_T_b.block<3,3>(0,0) = aux_function.W.rotmatrix(Eigen::Vector3d(0,0,M_PI)) * quat2dc(aux_function.W.Root.segment(3,4));  // SHOULD USE EITHER IMU(ROBOT) OR ROOT (SIM)
		wld_T_b.block<3,1>(0,3) = aux_function.sens_pos.head(3);
