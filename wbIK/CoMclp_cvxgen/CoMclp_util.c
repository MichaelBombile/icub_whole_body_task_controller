...
float CoMclp_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*CoMclp_ran1(idum, reset)-1.0;
      v2 = 2.0*CoMclp_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double CoMclp_randn(void) {
  return CoMclp_randn_internal(&CoMclp_global_seed, 0);
}
void CoMclp_reset_rand(void) {
  srand(15);
  CoMclp_global_seed = 1;
  CoMclp_randn_internal(&CoMclp_global_seed, 1);
}
