...
float wbIK_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*wbIK_ran1(idum, reset)-1.0;
      v2 = 2.0*wbIK_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double wbIK_randn(void) {
  return wbIK_randn_internal(&wbIK_global_seed, 0);
}
void wbIK_reset_rand(void) {
  srand(15);
  wbIK_global_seed = 1;
  wbIK_randn_internal(&wbIK_global_seed, 1);
}
