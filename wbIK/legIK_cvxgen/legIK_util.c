...
float legIK_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*legIK_ran1(idum, reset)-1.0;
      v2 = 2.0*legIK_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double legIK_randn(void) {
  return legIK_randn_internal(&legIK_global_seed, 0);
}
void legIK_reset_rand(void) {
  srand(15);
  legIK_global_seed = 1;
  legIK_randn_internal(&legIK_global_seed, 1);
}
